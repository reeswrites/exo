...
- `src/index.js` — JSON-RPC, auth, the brief and procedure resources, the audit and caller telemetry
- `src/tools.js` — the 31 tools, paging, and the byte ceiling
...
The grade stamps every answer and sizes nothing. There is one ceiling, 16KB per
call, and it is a context budget rather than a security control: it bounds how
much of the caller's window one answer consumes, whatever the grade of the rows
in it (ADR-0028). The row caps that once followed the grade are gone.

Every list tool takes `limit` and `offset`. `limit` is a page size — 20 unless
the caller says otherwise, 500 at most, and bytes bind first. `offset` starts
the page later. `tools/list` is built per request so the schema a model is told
about is the real one. See [Paging](#paging).
...
prefixed `wh_` are protected: that is where the call telemetry lives, and it is
the one thing on the box that is about the surface rather than the record — a
reconcile that dropped it would lose the history of what was asked.
...
write path in the surface to revoke (ADR-0006). What it can see is decided at
publish time, not here — the bundle physically lacks the held material
(ADR-0005), so nothing Poke asks can widen it. Every call answers within 16KB
and lands in the audit table, so you can see what was asked.
...
Fixed and semantic. There is no raw SQL and no free-form query, because the
surface is a data layer — a vocabulary of named questions whose answers are
facts about the record (ADR-0013) — not because a question is dangerous to ask.
What a question can reach is bounded by the serve projection, not by the
question's shape (ADR-0028). Adding one is a decision about what the record
answers well, and a name a model has learned is hard to take back.
...
half the problem: it says which end of a list a page came from, and it still
cannot say a page *of what*. Where a tool's rows are drawn from a
...
reads the notes and is stamped `private`; `taste` alone reads the scrobbles and
is stamped `profile`. And a field whose values are a closed hand-kept vocabulary
...
One ceiling: **16KB per call**, bytes before rows. It is a budget for the
caller's context window, not a bound on the corpus (ADR-0028), and it is the
same at every grade.

- `limit` — page size. Default 20; the caller may raise it, to 500 at most. A
  page that would exceed 16KB is clipped at the byte ceiling and says so.
- `offset` — where the page starts. Accepted by every list tool. Every
  `ORDER BY` carries a unique tiebreaker, so two pages of one question neither
  overlap nor skip a row — the prerequisite for offset paging to be *correct*,
  and the reason it was not offered until it was.
- `id` — the three tools that return one document whole (`notes_on`, `posts`,
  `drafts`) resolve a `topic` by vector search unless given an `id`. Every title
  listing carries the id, so a caller can go from a list to one document
  without searching for it again.

The envelope echoes what it did: `returned_count`, `offset`, `has_more`, and
`order` where the tool offers a choice. A caller that pages reads `has_more`;
one page is not the set.

Batched JSON-RPC is not supported — unimplemented, not refused on principle. A
batch is an error, never a partial answer.

Every call is logged to `wh_audit`: tool, arguments, rows returned, which door
it came through and from where. That is telemetry about what the surface is
used for, and it is the owner's to read.

## Who is calling

Two tables answer what was asked, by which door, and from where. They are
telemetry — a record of how the surface is used — and not a gate. The origin
allowlist ADR-0010 held open as its phase 2 is retired with the threat model it
served (ADR-0028). The observation stays, because it is cheap and because a
surface whose owner cannot see its own traffic is one whose next decision about
it is a guess.

`Origin` is not the signal. Poke and every other assistant call this
server-to-server — no Origin header, no preflight. What exists is the IP and
the ASN, which Cloudflare fills in `req.cf` before the isolate runs.

Both are `wh_`-prefixed so `import.sh` will not reconcile them away:

- **`wh_callers`** — where calls come from, rolled up per day per
  (ip, asn, ua, outcome). `outcome='ok'` is a client doing its job;
  `outcome='denied'` is a wrong token, which is misconfiguration far more often
  than anything else — `denied:authorize` is the one that means guessing.
  Rolled up rather than appended because the question is "which distinct places
  call", and because an append-only table on a path anyone can reach is a
  billing lever. Denials are additionally throttled to one write per ip per
  minute per isolate.
- **`wh_audit`** — the per-call log: tool, arguments, timestamp, rows returned,
  door, `ip` and `asn`. With more than one client connected (the owner's own
  machine, plus whatever assistant), "what was asked" without "by whom" cannot
  say which one asked it.
...
The first says which clients actually use the surface and how much; the second
says who is knocking with the wrong key. Neither feeds a gate. They answer the
question the log exists for — what does this surface get asked, and is it what
you built it for.
...
the root, and the header secret does not open `/mcp`. Both open the same
surface, and a holder of either is trusted with everything it serves
(ADR-0028) — what is served was decided by the projection before either door
existed. The log records which door a call came through, so the telemetry can
tell the two apart even though the surface does not.

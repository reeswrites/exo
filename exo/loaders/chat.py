...
— his turns graduate into `t1_notes` (raw) via promotion, world turns are
external — so it sits at the t0 input layer, grounds=false, per-row author (human
for his turns, external for world). The GROUNDS-AWARE WALL keeps it out of
...
def _landed(his_texts: list[tuple[int, str]], limit: int = 420) -> str:
    """His last substantial turn — the closest thing to a conclusion he wrote."""
    if not his_texts:
        return ""
...
def _digest(title: str, his_texts: list[tuple[int, str]], limit: int = 1400) -> str:
    """Title + a spread of his turns, for embedding. bge-small truncates around
...
def topics() -> list[Row]:
    ...
    Open threads record what he wrote down; this records what he worked through
    out loud, which is a different and fresher signal. The vault lags a
    deliberate act of capture; a conversation happens whether or not he files it.
    ...
    author=machine; they are labels, not his prose.
    """
    ...
    # digest is embedded — his words. The generated summary, when one exists,
